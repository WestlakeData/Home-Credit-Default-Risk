...
[XGBoost Final}(https://github.com/WestlakeData/Home-Credit-Default-Risk/blob/main/xgboost_final.R) contains the process for tuning the algorithm hyperparameters. [XGBoost Single](https://github.com/WestlakeData/Home-Credit-Default-Risk/blob/main/xgboost_final_single.R) has the final model trained on the tuned hyperparameters, which is used for evaluation of model performance, and eventual use on the test data set for submission to Kaggle.
