...
print(output)
### Training AUC: 0.7705759, Testing AUC: 0.7463063 -->  the logistic regression model shows a moderate level of discrimination between the classes, with better performance on the 
### training data compared to the testing data.
```
...
ggplot(roc_data, aes(x = TPR, y = FPR)) +
    ...
    theme_minimal()
## Higher AUC value on the training data compared to the testing data. This indicates that the model performs relatively well in distinguishing between positive and negative cases, but 
## there might be some drop in performance when applied to unseen data
```
...
### Based on the analysis of the logistic regression model, the variables Flag_Document 8/3 have the most significant impact on the outcome values. These variables, along with the Ext 
### source 3,2,1 variables from the application dataset, play an important role in predicting the outcomes.

### While the exact sources of the external data (Ext source 3,2,1) are not specified by Home Credit, these variables prove to be valuable in predicting the outcomes. Therefore, it is 
### recommended that the company collects more data from these external data sources (1, 2, and 3) to further improve the accuracy of the predictions.
...
### Overall, based on the analysis, it is recommended to focus on gathering more data from the application dataset and the specified external data sources (1, 2, and 3) to enhance the 
...
# tuned_rf <- train(TARGET ~ ., data = dt.train, method = "rf",
                  # metric = "Kappa", trControl = ctrl,
                  # tuneGrid = grid_rf)

# tuned_rf

# After several failed attempts at training the tuned Random Forest, I do not think my computer is able to handle the processing load. I did, however, leave my code for reference.
```
...
The model identified EXT_SOURCE_2, avg_days_credit, and avg_days_credit_enddate to be important. However, the model is extremely unspecific based on its TNR. The model did perform slightly better on the training dataset. It is likely overfitting and will need to have some parameters tweaked.
